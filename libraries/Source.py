import pipes, Project, os, shlex, subprocess

def project(target, configuration, directory, needy):
	if 'source-directory' in configuration or os.path.exists(os.path.join(directory, 'source')) or os.path.exists(os.path.join(directory, 'src')):
		return SourceProject(target, configuration, directory, needy)
	return None
...
class SourceProject(Project.Project):
	def __init__(self, target, configuration, directory, needy):
		...
		if 'source-directory' in self.configuration:
			self.source_directory = self.configuration['source-directory']
